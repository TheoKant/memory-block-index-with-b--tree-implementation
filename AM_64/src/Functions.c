...
int print_full_block(int fd,int bn){
        BF_Block *block;
        BF_Block_Init(&block);
        int s1=AM_File[fd]->size1, s2=AM_File[fd]->size2;
        char* data;
	...
	float key3;
	int counter = 0;
        if(BF_GetBlock(AM_File[fd]->fileDesc,bn,block) != BF_OK)
                return AME_ERROR;
        data = BF_Block_GetData(block);
	char c;
	...
	if(c != AM_DATA_ID)
		return 1;
	int c2;
	memcpy(&c2,data + sizeof(char) + sizeof(int),sizeof(int));
	while(counter != c2){
		if(AM_File[fd]->type1 == STRING){
			...
			memcpy((char*)&key2,data + sizeof(char) + 2*sizeof(int)+ counter*(s1+s2),s1);
			printf("%d->",key2);
		}
		else{
			memcpy((char*)&key3,data + sizeof(char) + 2*sizeof(int)+ counter*(s1+s2),s1);
			printf("%f->",key3);
		}
		counter++;
	}
...
int printData(int fd){
	...
	do{
		printf("%d:",next);
		print_full_block(fd,next);
		next = getNext(fd,next);
	}while(next != -1);
	return AME_OK;
}
